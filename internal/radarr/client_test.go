...
func TestName(t *testing.T) {
	c, err := NewRadarrClient("http://localhost:8989", "key", "", "", nil)
	if err != nil {
...
// NewRadarrClient must wire the credential fields onto the client exactly as
...
func TestNewRadarrClient_StoresCredentials(t *testing.T) {
	c, err := NewRadarrClient("http://localhost:8989", "my-api-key", "my-user", "test-password-not-real", nil)
	if err != nil {
		t.Fatalf("NewRadarrClient: %v", err)
	}
...
// mockArrServer is a configurable stand-in for the Radarr API surface that
...
func (m *mockArrServer) handleLogin(w http.ResponseWriter, r *http.Request) {
	...
	if valid && m.loginSetCookie {
		http.SetCookie(w, &http.Cookie{Name: "RadarrAuth", Value: "session-token"})
	}
...
func TestBackup_Success(t *testing.T) {
	...
	c, err := NewRadarrClient(m.srv.URL, "test-api-key", "", "", nil)
	if err != nil {
		t.Fatalf("NewRadarrClient: %v", err)
	}
...
func TestBackup_NoBackupsFound(t *testing.T) {
	...
	c, _ := NewRadarrClient(m.srv.URL, "test-api-key", "", "", nil)
	_, _, err := c.Backup(context.Background())
	if err == nil {
...
func TestBackup_PostgresDetected_OverrideUsedWhenConfigXMLEmpty(t *testing.T) {
	...
	c, _ := NewRadarrClient(m.srv.URL, "test-api-key", "", "", override)
	result, reader, err := c.Backup(context.Background())
	if err != nil {
		t.Fatalf("Backup() error: %v", err)
	}
...
func TestBackup_AuthMethodBasic_SendsBasicAuth(t *testing.T) {
	...
	c, _ := NewRadarrClient(srv.URL, "test-api-key", "myuser", "mypass", nil)
	_, reader, err := c.Backup(context.Background())
	if err != nil {
		t.Fatalf("Backup() error: %v", err)
	}
...
func TestBackup_AuthMethodForms_LoginSucceeds(t *testing.T) {
	...
	c, _ := NewRadarrClient(m.srv.URL, "test-api-key", "admin", "secret", nil)
	_, reader, err := c.Backup(context.Background())
	if err != nil {
		t.Fatalf("Backup() error: %v", err)
	}
...
func TestBackup_AuthMethodForms_InvalidCredentials(t *testing.T) {
	...
	c, _ := NewRadarrClient(m.srv.URL, "test-api-key", "admin", "wrong", nil)
	_, _, err := c.Backup(context.Background())
	if err == nil {
...
func TestBackup_APIKeyHeaderReachesServer(t *testing.T) {
	...
	c, err := NewRadarrClient(srv.URL, "test-api-key", "", "", nil)
	if err != nil {
		t.Fatalf("NewRadarrClient: %v", err)
	}
	_, reader, err := c.Backup(context.Background())
...
func TestBackup_WrongAPIKeyRejected(t *testing.T) {
	...
	c, err := NewRadarrClient(m.srv.URL, "wrong-key", "", "", nil)
	if err != nil {
		t.Fatalf("NewRadarrClient: %v", err)
	}
	_, _, err = c.Backup(context.Background())
...
func TestRestore_RestartRequired_TriggersRestart(t *testing.T) {
	...
	c, _ := NewRadarrClient(srv.URL, "test-api-key", "", "", nil)
